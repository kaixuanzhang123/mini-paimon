...
lsmTree.put(new Row(new Object[]{"error occurred", 1699000000L}));  // 允许
...
System.out.println("Recovered " + recovered.size() + " rows");  // 输出: 2
tree2.close();
```
...
**Q: 数据写入后重启丢失？**
A: 检查是否正常调用 `close()` 或 WAL 是否正确恢复。

**Q: 数据重复？**
A: 已修复。确保使用最新代码，WAL 在 flush 后会自动删除。

**Q: Compaction 未触发？**
A: Level 0 需要至少 4 个文件才会触发。可以插入更多数据测试。

**Q: 查询性能慢？**
A: 检查 Bloom Filter 是否生效，索引是否正确加载。
...
1. **添加新数据类型**
   - 修改 `DataType.java` 枚举
   - 更新 `RowKey.java` 序列化逻辑
   - 更新 `Row.java` 验证逻辑

2. **优化 Compaction**
   - 调整 `Compactor.java` 中的触发阈值
   - 实现并行 Compaction
   - 添加增量合并策略
